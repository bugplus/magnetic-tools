...
class SimpleCalibrationTester(QWidget):
    ...
    def run_test(self):
        try:
            ...
            # 角度转弧度（pitch/roll是角度，需要转换为弧度）
            pitch1 = np.radians(pitch1)
            roll1 = np.radians(roll1)
            pitch2 = np.radians(pitch2)
            roll2 = np.radians(roll2)
            
            # 2. 倾角补偿（正确的坐标系修正：6轴x对地磁-x，6轴y对地磁-y）
            mxh1, myh1 = 
            for i in range(len(mx1)):
                # 坐标系修正：6轴与地磁放置 x对-x，y对-y，z对z
                mx_corr = -mx1[i]  # 6轴x对地磁-x
                my_corr = -my1[i]  # 6轴y对地磁-y
                mz_corr = mz1[i]   # 6轴z对地磁z
            ...
            for i in range(len(mx2)):
                mx_corr = -mx2[i]  # 6轴x对地磁-x
                my_corr = -my2[i]  # 6轴y对地磁-y
                mz_corr = mz2[i]   # 6轴z对地磁z
